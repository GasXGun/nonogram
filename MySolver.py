...
# ======================================================
# 強力解析器 (Robust Parser) - 保持不變
# ======================================================
def parse_line_robust(line_str):
    numbers = re.findall(r'\d+', line_str.split('#')[0])
...
def main():
    ...
    with open(output_file, 'w') as f_out:
        ...
        while idx < len(lines):
            if '$' in lines[idx]:
                pid_match = re.search(r'\$(\d+)', lines[idx])
                pid = f"${pid_match.group(1)}" if pid_match else lines[idx]
                ...
                # Read 25 Col Hints
                count = 0
                while count < 25 and idx < len(lines):
                    line = lines[idx]
                    idx += 1
                    if not line: continue
                    hints = parse_line_robust(line)
                    hints = [h for h in hints if h <= 25] 
                ...
                if len(col_hints) == 25 and len(row_hints) == 25:
                    ...
                    print(f"-> Error: Hint count mismatch.")
                    f_out.write("Unsolvable\n")
